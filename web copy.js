...
let address = "0x36e61faDa873b03f4a2ec88B3715928d32ae0a66";
...
let userpatch = async () => {
	...
	patches = patches.filter(async (val)=>{
		let pchname = val["patchname"];
		let arr = await contract.methods.patchhistory(pchname).call();
		arr = arr.filter((val)=>{
			return val.usname == user["username"];
		});
		if(arr.length == 0){
			return true;
		}
...
let report = () => {
	...
	// let r1={"bugs":bugs,"features":features};
	// reports[document.getElementById("Software").value]=r1;
	contract.methods.userreport(document.getElementById("Software").value, bugs, features).send({ from: account }).then(() => {
...
// contract.methods.labeller("Software1").call().then((res) => {
// 	console.log(res);
// })
let commit = () => {
	if (Object.keys(bugspr).length == 0 && Object.keys(featurespr).length == 0) {
		...
		contract.methods.prioritychange(document.getElementById("Software").value,Object.keys(bugspr), Object.values(bugspr), Object.keys(featurespr), Object.values(featurespr)).send({ from: account }).then(() => {
			alert("Commited changes");
			window.location.reload();
		})
	}
}
...
let Request_patch = () => {
	...
	if (bgind.length != 0 && ftind.length != 0) {
		...
		contract.methods.requestpatch(software, bgind, ftind).send({ from: account }).then(() => {
			alert("Request sent");
			window.location.reload();
		})
	}
	else {
		alert("Send both features and Bugs for the patch");
	}
}
